...
public:
  ...
  UniquePtr<std::void_t< std::is_pointer_v<Args> && std::is_arithmetic<Args>::value && ...>>>(
      Args &&...arg) noexcept
      : ptr(new T(std::forward<Args>(arg)...)), del() {}
  ...
  UniquePtr<U, std::void_t<typename std::is_pointer<T *>::value>>(T *ptr,
                                                                  D deleter)
      : ptr(ptr), del(deleter){};
